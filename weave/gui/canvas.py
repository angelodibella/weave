...
def _is_valid_connection(source: dict[str, Any], target: dict[str, Any]) -> bool:
    ...
    if source["type"] in quantum_types and target["type"] in quantum_types:
        return (
            source["type"] == "qubit"
            and target["type"] in {"Z_stabilizer", "X_stabilizer"}
        ) or (
            target["type"] == "qubit"
            and source["type"] in {"Z_stabilizer", "X_stabilizer"}
        )
...
class Canvas(QWidget):
    ...
    def __init__(self, parent=None, dark_mode=False):
        ...
        self.graphs = (
            []
        )  # each entry: {'node_ids': set(), 'type': 'quantum' or 'classical', 'selected': bool}
        self.graph_drag = None  # Reference to the graph being dragged
        self.graph_drag_initial_positions = (
            {}
        )  # Store initial positions of nodes in the graph

        # Selection state.
        self.selecting = False  # whether a rectangular selection is active
        self.selection_rect_start = (
            None  # the starting world coordinate of the selection
        )
        self.selection_rect = (
            None  # current selection rectangle (x_min, y_min, x_max, y_max)
        )
        ...
        self._drag_start_positions = (
            {}
        )  # dictionary to store initial positions of selected nodes when starting a drag

        # Selection state.
        self.shift_pending_toggle = (
            None  # will hold a reference to the node pending deselection
        )
        self._shift_press_node = None  # store node clicked with shift
        self._shift_press_pos = None  # store position of shift-click
        self._shift_press_was_selected = (
            False  # store node's selection state before shift-click
        )
    ...
    def _create_hamburger_menu(self):
        ...
        self._hamburger_menu.setWindowFlags(
            self._hamburger_menu.windowFlags() | Qt.FramelessWindowHint
        )
        ...
        self.dark_mode_toggle = ToggleSwitch(
            self.theme_manager.dark_mode, dark_mode_widget
        )
        ...
        clear_action = self._hamburger_menu.addAction(
            "Clear Canvas", self._clear_canvas
        )
        ...
        pos = QPointF(
            hamburger_bottom_right.x() - self._hamburger_menu.sizeHint().width(),
            hamburger_bottom_right.y() + 5,
        ).toPoint()
    ...
    def contextMenuEvent(self, event):
        ...
        menu.addAction(
            "New Parity Check", lambda: self.add_node_at(event.pos(), "parity_check")
        )
        ...
        quantum_menu.addAction(
            "New Qubit", lambda: self.add_node_at(event.pos(), "qubit")
        )
        quantum_menu.addAction(
            "New Z-Stabilizer", lambda: self.add_node_at(event.pos(), "Z_stabilizer")
        )
        quantum_menu.addAction(
            "New X-Stabilizer", lambda: self.add_node_at(event.pos(), "X_stabilizer")
        )
        ...
        if node:
            ...
            menu.addAction(
                "Save Code as CSV",
                lambda: print("Save functionality not implemented yet."),
            )
        ...
        if event.key() == Qt.Key_0 and event.modifiers() & Qt.ControlModifier:
            ...
            if selected_nodes:
                ...
                self.edges = [
                    e
                    for e in self.edges
                    if e["source"] not in ids_to_remove
                    and e["target"] not in ids_to_remove
                ]
                self._deselect_all()
                self._update_graphs()
            else:
                ...
                if selected_edges:
                    ...
                    selected_graphs = [
                        g for g in self.graphs if g.get("selected", False)
                    ]
                    if selected_graphs:
            ...
            if clicked_node:
                if event.modifiers() & Qt.ControlModifier:
                    ...
                    if selected_nodes:
                        for sn in selected_nodes:
                            if sn != clicked_node and _is_valid_connection(
                                sn, clicked_node
                            ):
                    ...
                    self._drag_start_positions = {
                        n["id"]: n["pos"] for n in self._get_selected_nodes()
                    }
                    ...
                    clicked_node["selected"] = True
                    self._drag_start_positions = {
                        clicked_node["id"]: clicked_node["pos"]
                    }
                    self.drag_start = event.position()
                self.update()
            else:
                ...
                if clicked_edge:
                    ...
                    if clicked_graph:
                        ...
                        self._drag_start_positions = {}
                        world_pos = (
                            (pos.x() - self._view_offset.x()) / self._zoom,
    ...
    def save_to_file(self, filename):
        ...
        data = {
            "nodes": [
                {k: v for k, v in node.items() if k != "selected"}
                for node in self.nodes
            ],
            "edges": [
                {k: v for k, v in edge.items() if k != "selected"}
                for edge in self.edges
            ],
    ...
    def _draw_edges(self, painter):
        for edge in self.edges:
            ...
            if source["type"] in {"bit", "parity_check"} and target["type"] in {
                "bit",
                "parity_check",
            }:
    ...
    def _draw_crossings(self, painter):
        ...
        quantum_types = {"qubit", "Z_stabilizer", "X_stabilizer"}
        qnodes = {
            node["id"]: node for node in self.nodes if node["type"] in quantum_types
        }
        qedges = [
            edge
            for edge in self.edges
            if self.get_node_by_id(edge["source"])
            and self.get_node_by_id(edge["target"])
        ...
        for node in self.nodes:
            ...
            if is_selected:
                ...
                if node_type == "bit" or node_type == "qubit":
                    painter.drawEllipse(
                        QPointF(x, y), r + highlight_size, r + highlight_size
                    )
                else:
    ...
    def _update_graphs(self):
        ...
        for graph in self.graphs:
            ...
            any_node_id = next(
                iter(graph["node_ids"] & {n["id"] for n in self.nodes}), None
            )
    ...
    def _save_canvas(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Canvas", "", "JSON Files (*.json)"
        )
    ...
    def _load_canvas(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Load Canvas", "", "JSON Files (*.json)"
        )
    ...
    def _copy_selected(self):
        ...
        if self.clipboard_nodes:
            avg_x = sum(node["pos"][0] for node in self.clipboard_nodes) / len(
                self.clipboard_nodes
            )
            avg_y = sum(node["pos"][1] for node in self.clipboard_nodes) / len(
                self.clipboard_nodes
            )
